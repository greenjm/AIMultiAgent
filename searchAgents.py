...
def cornersHeuristic(state, problem):
  ...
  for corner in corners:
    if corner not in visitedCorners:
      result += util.manhattanDistance(position, corner)
      position = corner
  return result
...
def foodHeuristic(state, problem):
  ...
  "*** YOUR CODE HERE ***"
  if problem.heuristicInfo.get('foodList') is None:
  ...
  for food in foodList:
    if foodGrid[food[0]][food[1]]:
      euclidDistance = ( (position[0] - food[0]) ** 2 + (position[1] - food[1]) ** 2 ) ** 0.5
      result += euclidDistance
      position = food
  return result
...
class AnyFoodSearchProblem(PositionSearchProblem):
  ...
  def isGoalState(self, state):
    ...
    for food in foodList:
      if self.food[food[0]][food[1]] and state == food:
        return True
    return False
